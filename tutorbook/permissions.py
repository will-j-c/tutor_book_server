...
class IsOwner(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        try:
            # Case 4: Check the user owns the assignment
            print('Checking that requestor owns the assignment')
            request_user_id = int(request.user.pk)
            assignment_user_id = int(obj.user_id)
            has_permission = request_user_id == assignment_user_id

            if has_permission:
                print('Assignment level permission granted')
                return has_permission
